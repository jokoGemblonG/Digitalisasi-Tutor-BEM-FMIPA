...
<body>
	...
	<tbody>
		<?php
		$query = pg_query("SELECT K.idk AS idk, hari, jam, nama FROM (((kelas K INNER JOIN sesi S ON K.ids = S.ids) INNER JOIN tutor T ON K.id_tutor = T.id_tutor) INNER JOIN mata_kuliah M ON K.idm = M.idm) WHERE K.idm = 'MBL'");
